...
network_lst = ['yeast', 'GSE', 'ca-GrQc', 'ca-CondMat', 'powergrid']
metrics = ['internal density', 'cut ratio', 'conductance']
...
for i, row in enumerate(axes):
	...
	for j, cell in enumerate(row):
		cell.boxplot(metric_lst[j], showmeans=True)
		if i == 0:
...
fig, axes = plt.subplots(nrows=len(metrics), ncols=len(network_lst),
...
for i in range(len(network_lst)):
	...
	for j in range(len(metrics)):
		axes[j, i].boxplot(metric_lst[j], showmeans=True)
		if j == 0:
			#axes[j, i].set_ylabel(metrics[j])
			axes[j, i].set_title(network_lst[i])
		if i == 0:
			axes[j, i].set_ylabel(metrics[j])
			#axes[j, i].set_ylabel(network_lst[i])
